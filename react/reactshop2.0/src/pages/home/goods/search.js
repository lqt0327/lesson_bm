...
export default class  GoodsSearch extends React.Component{
    ...
    getPageData(){
        this.setParams();
        let url=config.baseUrl+"/home/goods/search?"+this.sParams+"&page=1&token="+config.token;
    ...
    getScrollPage(){
        this.oUpRefresh=new UpRefresh({"curPage":this.curPage,"maxPage":this.maxPage,"offsetBottom":this.offsetBottom},curPage=>{
            let url=config.baseUrl+"/home/goods/search?"+this.sParams+"&page="+curPage+"&token="+config.token;
            request(url).then((res)=>{
                if (res.code===200){
    ...
    //获取分类数据
    getClassifyData(){
        let url=config.baseUrl+"/home/category/menu?token="+config.token;
        request(url).then(res=>{
            if (res.code ===200){
                let aClassify=this.state.aClassify;
    ...
    //获取属性数据
    getAttrData(){
        let url=config.baseUrl+"/home/goods/param?kwords="+this.sKeywords+"&token="+config.token;
        request(url).then(res=>{
            if (res.code ===200){
                let aAttr=res.data;
